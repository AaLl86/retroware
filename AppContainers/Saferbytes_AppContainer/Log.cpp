 ...
 *  Revision History:
 *	20/02/12 - Added version information routine
 *	Copyright© 2012 
 */
#include "stdafx.h"
#include "log.h"
...
bool CLog::Open(LPTSTR fileName, bool overwrite) {
	...
	if (!logTitle) {
		// 20/02/2012 If there isn't a log title use CVersionInfo
			...
			verStr = fileInfo.GetFileVersionString(false);
...
void CLog::Write(LPWSTR dbgStr) {
	...
	if (!this->g_hLogFile || g_hLogFile == INVALID_HANDLE_VALUE) {		// If I don't have opened a log file
		...
		retVal = WriteFile(g_hLogFile, logStr, bytesToWrite - 1, &bytesWritten, NULL);
		delete[] logStr;
	}
...
void CLog::Write(LPSTR dbgStr) {
	...
	if (!this->g_hLogFile || g_hLogFile == INVALID_HANDLE_VALUE) {		// If I don't have opened a log file
		...
		retVal = WriteFile(g_hLogFile, dbgStr, bytesToWrite, &bytesWritten, NULL);
	}
	g_bAtLeastOneWrite = true;
}
...
CVersionInfo::CVersionInfo(LPTSTR modName) {
	...
	g_fVerStr = NULL;
	bool retVal = false;
	retVal = GetModuleVersionInfo(modName);
}
...
LPTSTR CVersionInfo::GetFileVersionString(bool bClassic) {
	...
	if (bClassic) {
		...
		// New AaLl86 version string
		WORD build = (WORD)fixedVer.dwFileVersionLS;
		TCHAR buildStr[0x10] = {0};
		if (!build) buildStr[0] = 0;
		else if (build <= 2) wsprintf(buildStr, L" %s %i", L"Alpha", build);
		else if (build > 2 && build <= 9) wsprintf(buildStr, L" %s %i", L"Beta", build - 2);
		else if (build >= 10) wsprintf(buildStr, L" %s %i", L"Build", build);

		wsprintf(g_fVerStr, L"%i.%i.%i",
			(WORD)(fixedVer.dwFileVersionMS >> 16),
			(WORD)(fixedVer.dwFileVersionMS),
			(WORD)(fixedVer.dwFileVersionLS >> 16));
		if (buildStr[0]) wcscat_s(g_fVerStr, 0x30, buildStr);
...
LPTSTR CVersionInfo::QueryVersionValue(LPTSTR valueName) {
	return VerQueryLangCpValue(valueName);
}
