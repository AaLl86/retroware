...
bool CPeResource::Save(LPCTSTR resName, LPCTSTR resType, LPTSTR fileName, bool Overwrite) {
	...
	if (hFile == INVALID_HANDLE_VALUE) {
		TCHAR dbgStr[200] = {0};
		swprintf_s(dbgStr, COUNTOF(dbgStr), L"CPeResource::Save - CreateFile has failed with error code %i.", (LPVOID)lastErr);
...
HGLOBAL CPeResource::ExtractGlobal(LPCTSTR resName, LPCTSTR resType, DWORD * sizeOfRes) {
	...
	if (!hResData || !hRes) {
		TCHAR dbgStr[200] = {0};
		swprintf_s(dbgStr, COUNTOF(dbgStr), L"CPeResource::Extract - FindResource or LoadResource has failed with error code: 0x08X.", (LPVOID)GetLastError());
		OutputDebugString(dbgStr);
		return NULL;
	}
	return hResData;
